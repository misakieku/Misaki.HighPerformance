    ...
    where T : unmanaged
{
    ...
    /// Use <see cref="UnsafeArray(int, AllocationHandle, AllocationOption)"/> constructor and <see cref="MemoryUtility.MemCpy(void*, void*, nuint)"/> if you are not sure what you are doing.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// The returned <see cref="UnsafeArray{U}"/> shares the same memory as the original array, and does not own the memory.
    /// </remarks>
    /// <typeparam name="U">The unmanaged type to reinterpret the buffer as.</typeparam>
    /// <returns>An <see cref="UnsafeArray{U}"/> that views the same memory as the original array, but as elements of type U.</returns>
